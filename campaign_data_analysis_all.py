...
for plt_i, experiment_dir in enumerate(experiment_dirs):
    ...
    if show_human_change:
        ...
        axs[plt_i].axhline(y=human_change_24_28[metric_human], color=human_data_color, linestyle=':', zorder=0)
        axs[plt_i].text(human_change_xloc, human_change_24_28[metric_human] + 0.01, "Human value stability between ages 24 and 28",
                        fontsize=human_data_fontsize, color=human_data_color)

        axs[plt_i].axhline(y=human_change_20_24[metric_human], color=human_data_color, linestyle=':', zorder=0)
        axs[plt_i].text(human_change_xloc, human_change_20_24[metric_human] + 0.01, "Human value stability between ages 20 and 24",
                        fontsize=human_data_fontsize, color=human_data_color)
...
if families_plot:
    ...
    colors = [family_2_color.get(f, "black") for f in families]
    ...
    plt.gca().set_xticklabels(families, rotation=rotatation_x_labels, fontsize=xticks_fontsize, rotation_mode="anchor")

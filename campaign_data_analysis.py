...
def run_analysis(eval_script_path, data_dir, assert_n_contexts=None, paired_data_dir=None, no_ips=False):
    ...
    command = f"python {eval_script_path} --result-json-stdout {'--assert-n-dirs ' + str(assert_n_contexts) if assert_n_contexts else ''} {data_dir}/* {data_dir}/*/* {f'--paired-dirs {paired_data_dir}/*' if paired_data_dir is not None else ''} {'--no-ips' if no_ips else ''}"

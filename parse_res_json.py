...
def process_json(json_path, show_values=False):
    ...
    for i in range(len(answers)):
        # all_ans.append([x for x, _ in answers[i]['pvq_male']])
        all_ans.append([x for x, _ in list(answers[i].values())[0]])
    ...
    if show_values:
        for metr, part in zip(json_data['per_simulated_participant_metrics'], json_data["simulated_population"]):
            # if "Stalin" in part or "Mandela" in part or "Monroe" in part:
            if part in ["Morgoth", "Sauron", "Gandalf", "Frodo Baggins", "Peregrin Took"]:
                print("---------------")
                print(part)
                print(" ".join([f"{val}:{sc:.2f}" for val, sc in metr['pvq_male'].items()]))
